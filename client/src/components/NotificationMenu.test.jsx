...
vi.mock("@/api/client.js", () => ({ api: { get: vi.fn(), patch: vi.fn() } }));
...
describe("NotificationMenu", () => {
  it("loads notifications only when opened", async () => {
    ...
    expect(await screen.findByText("Universiteti u aktivizua")).toBeInTheDocument();
  ...
  it("closes the panel with Escape", async () => {
    ...
    render(<NotificationMenu />);
    await user.click(screen.getByRole("button", { name: "Njoftimet" }));
    expect(await screen.findByRole("region", { name: "Njoftimet e universitetit" })).toBeInTheDocument();
    await user.keyboard("{Escape}");
    expect(screen.queryByRole("region", { name: "Njoftimet e universitetit" })).not.toBeInTheDocument();
  });
});
