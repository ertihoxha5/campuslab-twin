...
export function TwinSelectionPanel({ selection,history,onClose,onFocus }) {
  ...
  const isSensor=selection.kind==="sensor";
  return <aside className="twin-selection-panel" aria-label="Detajet e objektit të zgjedhur">
    ...
    <dl className="twin-object-details">
      ...
      {isSensor?<><div><dt>Matja</dt><dd>{item.value} {item.unit}</dd></div><div><dt>Statusi</dt><dd>{item.state}</dd></div><div><dt>Asset-i</dt><dd>{item.assetId??"Ambient"}</dd></div></>:<><div><dt>Temperatura</dt><dd>{item.temperature.toFixed(1)} °C</dd></div><div><dt>Energjia</dt><dd>{item.energyWatts.toFixed(0)} W</dd></div><div><dt>Mirëmbajtja</dt><dd>{item.maintenance}</dd></div></>}
    </dl>
