...
function DeviceBody({ type, color }) {
  if (type === "smoke" || type === "occupancy") return <group rotation={[Math.PI/2,0,0]}><mesh castShadow><cylinderGeometry args={[.16,.19,.08,28]}/><meshPhysicalMaterial color="#E8E9E6" roughness={.32}/></mesh><mesh position={[0,.048,0]}><torusGeometry args={[.095,.018,10,24]}/><meshStandardMaterial color={color} emissive={color} emissiveIntensity={.55}/></mesh></group>;
  if (type === "door") return <group><RoundedBoxLike/><mesh position={[.09,0,0]}><boxGeometry args={[.055,.26,.06]}/><meshStandardMaterial color={color} emissive={color} emissiveIntensity={.5}/></mesh></group>;
...
export const IoTDevices = memo(function IoTDevices({ sensors, visible, selectedId, onSelect, showLabels }) {
  return <group>{sensors.map((sensor)=><SensorDevice key={sensor.id} sensor={sensor} visible={visible} selected={selectedId===sensor.id} onSelect={onSelect} showLabel={showLabels}/>)}</group>;
});

function SensorDevice({ sensor, visible, selected, onSelect, showLabel }) {
  ...
  return <group position={sensor.position} onClick={(event)=>{event.stopPropagation();onSelect(sensor);}} onPointerEnter={(event)=>{event.stopPropagation();setHovered(true);document.body.style.cursor="pointer";}} onPointerLeave={()=>{setHovered(false);document.body.style.cursor="default";}}>
    <group scale={selected||hovered?1.22:1}><DeviceBody type={sensor.type} color={color}/></group>
    ...
    {sensor.state!=="offline"&&<pointLight color={color} intensity={sensor.state==="warning"?.75:.2} distance={1.4}/>} 
