...
export function useAssetPlacement({ laboratoryId, placements, onSaved }) {
  ...
  const begin = useCallback((template={}) => { setOpen(true); setError(""); setDraft((current) => ({ ...INITIAL,...template,zoneId:String(template.zoneId??current.zoneId??zones[0]?.id??""),mountingSurface:template.mountingSurface??"floor",assetCode:template.assetCode??`TWIN-${Date.now().toString().slice(-6)}` })); }, [zones]);
