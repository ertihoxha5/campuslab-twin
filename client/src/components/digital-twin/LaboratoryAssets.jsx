...
const MODELS = {
  ...
  rack: `${ROOT}/server-rack.glb`, camera: `${ROOT}/security_camera_01/security_camera_01_1k.gltf`,
...
function Model({ src, height, rotation = 0 }) {
  ...
  const fit = useMemo(() => modelTransform(gltf.scene, height), [gltf.scene, height]);
  return <group rotation={[0,rotation,0]} scale={fit.scale}><group position={fit.offset}><Clone object={gltf.scene} castShadow receiveShadow /></group></group>;
}
function InteractiveAsset({ asset, selected, onSelect, children }) {
  ...
  return <group position={[asset.position[0],asset.position[1]+.025,asset.position[2]]} onClick={(e)=>{e.stopPropagation();onSelect(asset);}} onPointerEnter={(e)=>{e.stopPropagation();setHovered(true);document.body.style.cursor="pointer";}} onPointerLeave={()=>{setHovered(false);document.body.style.cursor="default";}}>
...
function Workstation(){return <group><Model src={MODELS.officeDesk} height={.76}/><group position={[0,.78,-.04]}><Model src={MODELS.laptop} height={.18}/></group><group position={[0,0,.57]} rotation={[0,Math.PI,0]}><Model src={MODELS.chair} height={.82}/></group></group>;}
function AssetModel({type}){if(type==="robot")return <RobotArm/>;if(type==="workstation")return <Workstation/>;if(type==="network-rack")return <Model src={MODELS.rack} height={2.05}/>;if(type==="storage-rack")return <Model src={MODELS.shelf} height={2.15} rotation={Math.PI/2}/>;if(type==="camera")return <Model src={MODELS.camera} height={.34} rotation={Math.PI}/>;if(type==="display")return <DashboardScreen position={[0,1.5,0]} wide/>;if(type==="alarm-panel")return <RoundedBox args={[.62,.82,.18]} radius={.04} position={[0,.82,0]}><meshStandardMaterial color="#D9DAD5"/></RoundedBox>;if(type==="emergency-stop")return <group><mesh position={[0,.57,0]}><cylinderGeometry args={[.045,.05,1.14,16]}/><meshStandardMaterial color="#D0A92E"/></mesh><mesh position={[0,1.13,0]}><cylinderGeometry args={[.14,.17,.14,24]}/><meshStandardMaterial color="#C53C3C"/></mesh></group>;return null;}
export const LaboratoryAssets=memo(function LaboratoryAssets({assets,visible,cameraVisible=true,selectedId,onSelect}){if(!visible&&!cameraVisible)return null;return <group>{assets.filter((asset)=>asset.type!=="generic"&&(asset.type==="camera"?cameraVisible:visible)).map((asset)=><InteractiveAsset key={asset.id} asset={asset} selected={selectedId===asset.id} onSelect={onSelect}><AssetModel type={asset.type}/></InteractiveAsset>)}</group>;});
