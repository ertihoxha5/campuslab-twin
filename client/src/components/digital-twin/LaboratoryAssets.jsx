...
function AssetModel({type}){if(type==="robot")return <Model src={MODELS.robot} height={1.15} rotation={-Math.PI/2}/>;if(type==="workstation")return <Model src={MODELS.officeDesk} height={.76}/>;if(type==="network-rack")return <Model src={MODELS.rack} height={2.05}/>;if(type==="storage-rack")return <Model src={MODELS.shelf} height={2.15} rotation={Math.PI/2}/>;if(type==="camera")return <Model src={MODELS.camera} height={.34} rotation={Math.PI}/>;if(type==="display")return <DashboardScreen position={[0,1.5,0]} wide/>;if(type==="alarm-panel")return <RoundedBox args={[.62,.82,.18]} radius={.04} position={[0,.82,0]}><meshStandardMaterial color="#D9DAD5"/></RoundedBox>;if(type==="emergency-stop")return <group><mesh position={[0,.57,0]}><cylinderGeometry args={[.045,.05,1.14,16]}/><meshStandardMaterial color="#D0A92E"/></mesh><mesh position={[0,1.13,0]}><cylinderGeometry args={[.14,.17,.14,24]}/><meshStandardMaterial color="#C53C3C"/></mesh></group>;return null;}
