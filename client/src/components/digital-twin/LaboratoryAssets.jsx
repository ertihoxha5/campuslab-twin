...
function AssetModel({type}){if(type==="robot")return <Model src={MODELS.robot} height={1.75} rotation={-Math.PI/2}/>;if(type==="workstation")return <Model src={MODELS.officeDesk} height={.76}/>;if(type==="network-rack")return <Model src={MODELS.rack} height={2.05}/>;if(type==="storage-rack")return <Model src={MODELS.shelf} height={2.15} rotation={Math.PI/2}/>;if(type==="camera")return <Model src={MODELS.camera} height={.34} rotation={Math.PI}/>;if(type==="display")return <DashboardScreen position={[0,1.5,0]} wide/>;if(type==="alarm-panel")return <RoundedBox args={[.62,.82,.18]} radius={.04} position={[0,.82,0]}><meshStandardMaterial color="#D9DAD5"/></RoundedBox>;if(type==="emergency-stop")return <group><mesh position={[0,.57,0]}><cylinderGeometry args={[.045,.05,1.14,16]}/><meshStandardMaterial color="#D0A92E"/></mesh><mesh position={[0,1.13,0]}><cylinderGeometry args={[.14,.17,.14,24]}/><meshStandardMaterial color="#C53C3C"/></mesh></group>;return null;}
function Station({position,rotation=0,office=false}){return <group position={position} rotation={[0,rotation,0]}><Model src={office?MODELS.officeDesk:MODELS.desk} height={.76}/><Model src={MODELS.laptop} height={.33}/><group position={[0,0,.68]} rotation={[0,Math.PI,0]}><Model src={MODELS.chair} height={.92}/></group></group>;}
function Furnishings(){const classStations=[[-7.65,-2.35],[-6.15,-2.35],[-4.65,-2.35],[-7.65,-4.25],[-6.15,-4.25],[-4.65,-4.25]];return <group>
  {[[-7.4,3.25],[-5.35,3.25]].map(([x,z])=><group key={x} position={[x,0,z]}><Model src={MODELS.officeDesk} height={.76}/><DashboardScreen position={[-.38,1.15,-.16]} rotation={.08}/><DashboardScreen position={[.38,1.15,-.16]} rotation={-.08}/><group position={[0,0,.78]} rotation={[0,Math.PI,0]}><Model src={MODELS.chair} height={.95}/></group></group>)}
  <DashboardScreen position={[-6.35,1.75,6.18]} wide/>{classStations.map(([x,z])=><Station key={`${x}:${z}`} position={[x,0,z]}/>)}<Station position={[-8.15,0,-5.15]} rotation={Math.PI/2} office/><DashboardScreen position={[-3.2,1.55,-3.8]} rotation={-Math.PI/2} wide/>
  <group position={[3.5,0,3.45]}><Model src={MODELS.officeDesk} height={.76}/><Model src={MODELS.drill} height={1.35}/></group><group position={[4.55,1.65,5.75]}><Model src={MODELS.camera} height={.3}/></group>
  {[[-.9,-3.2],[.9,-3.2],[-.9,-5.05],[.9,-5.05]].map(([x,z],i)=><group key={i} position={[x,0,z]}><Model src={i%2?MODELS.rack:MODELS.shelf} height={2.05} rotation={Math.PI/2}/></group>)}
  {[[4.1,-4.9],[7.65,-4.9]].map(([x,z])=><group key={x} position={[x,0,z]}><Model src={MODELS.extinguisher} height={.82}/></group>)}
  </group>;}
export const LaboratoryAssets=memo(function LaboratoryAssets({assets,visible,selectedId,onSelect}){if(!visible)return null;return <group>{assets.map((asset)=><InteractiveAsset key={asset.id} asset={asset} selected={selectedId===asset.id} onSelect={onSelect}><AssetModel type={asset.type}/></InteractiveAsset>)}<Furnishings/></group>;});
