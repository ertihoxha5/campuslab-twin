...
export const LaboratoryAssets=memo(function LaboratoryAssets({assets,visible,selectedId,onSelect}){if(!visible)return null;return <group>{assets.map((asset)=><InteractiveAsset key={asset.id} asset={asset} selected={selectedId===asset.id} onSelect={onSelect}><AssetModel type={asset.type}/></InteractiveAsset>)}</group>;});
