...
export function HowItWorksPage() {
  ...
  return <main className="how-page">
    <section className="how-hero"><div className="site-container"><div className="how-hero-copy"><p className="eyebrow">Si funksionon</p><h1>Nga hapësira te <em>vendimi live.</em></h1><p>Një rrjedhë e kontrolluar që e kthen laboratorin, pajisjet dhe sinjalet në një sistem operacional të kuptueshëm.</p><div><Button asChild size="lg"><Link to="/regjistrohu">Fillo me universitetin <ArrowRight size={18} /></Link></Button><a href="#rrugetimi">Shiko rrugëtimin <ArrowDown size={16} /></a></div></div><div className="how-hero-console"><div className="hero-console-head"><span><i /> PROCESI AKTIV</span><small>{stages[activeStage].number} / 06</small></div><div className="hero-console-stage" key={`hero-${stages[activeStage].number}`}><span><ActiveIcon /></span><div><small>{stages[activeStage].label}</small><strong>{stages[activeStage].title}</strong><p>{stages[activeStage].result}</p></div></div><div className="hero-console-track"><i style={{ width: `${((activeStage + 1) / stages.length) * 100}%` }} /></div><div className="hero-console-controls" aria-label="Zgjidh hapin e procesit">{stages.map((stage, index) => <button key={stage.number} type="button" className={activeStage === index ? "active" : ""} onClick={() => setActiveStage(index)} aria-label={`${stage.number}: ${stage.label}`}><span>{stage.number}</span><small>{stage.label}</small></button>)}</div><div className="hero-console-foot"><span>{stages[activeStage].owner}</span><button type="button" onClick={() => setActiveStage((activeStage + 1) % stages.length)}>Hapi tjetër <ArrowRight size={15} /></button></div></div></div></section>
