...
export function RealtimeMonitoringPage() {
  ...
  const loadSnapshot = useCallback(async () => {
    ...
    try {
        ...
        for (const [sensorId, reading] of Object.entries(current)) {
          if (
            ...
            new Date(reading.recordedAt) >=
              new Date(restored[sensorId].recordedAt)
          ) {
      ...
      setLastUpdate((current) =>
        current &&
        latestRecordedAt &&
        new Date(current) > new Date(latestRecordedAt)
          ? current
    ...
    return connectMonitoringRealtime({
      ...
      onEvent(eventName, payload) {
        if (eventName === "sensor:reading") {
          ...
          setLastUpdate(payload.recordedAt);
          setAnnouncement(
            `Lexim i ri i energjisë: ${Number(payload.powerWatts).toLocaleString("sq-AL")} vat.`,
          );
        } else if (["alert:created", "alert:updated"].includes(eventName)) {
          setAnnouncement(
            eventName === "alert:created"
              ? `Alarm i ri: ${payload.title ?? "kontrolloni alarmet aktive"}.`
              : "Statusi i një alarmi u përditësua.",
          );
          loadSnapshot();
        }
      },
  ...
  return (
    <section className="workspace-overview realtime-monitoring-page">
      ...
      {!networkOnline && (
        <div className="realtime-offline-note" role="note">
          Të dhënat e fundit mbeten të dukshme. Përditësimet vazhdojnë sapo të
          rikthehet interneti.
        </div>
      )}
      ...
      {message && (
        <div className="dashboard-error" role="alert">
          <strong>Të dhënat nuk mund të ngarkohen</strong>
          <p>{message}</p>
        </div>
      )}
      ...
      <div className="realtime-summary-grid">
        ...
        <article>
          <Zap size={20} />
          <span>Fuqia e fundit</span>
          <strong>
            {energy
              ? `${Number(energy.powerWatts).toLocaleString("sq-AL")} W`
              : "—"}
          </strong>
        </article>
        <article>
          <CircleAlert size={20} />
          <span>Alarme aktive</span>
        ...
        <article>
          ...
          <strong>
            {latestReadings.length
              ? latestReadings.some((reading) => reading.source === "simulated")
                ? "Simulim"
                : "Regjistruar"
              : "—"}
          </strong>
