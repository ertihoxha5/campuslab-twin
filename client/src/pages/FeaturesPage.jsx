...
export function FeaturesPage() {
  ...
  return <main className="ecosystem-page">
    <section className="ecosystem-hero"><div className="site-container"><div className="ecosystem-hero-copy"><p className="eyebrow">Platforma CampusLab Twin</p><h1>Një ekosistem.<br /><em>Çdo sinjal i lidhur.</em></h1><div className="ecosystem-hero-bottom"><p>Gjashtë shtresa operative punojnë si një sistem i vetëm—nga hapësira fizike te vendimi i dokumentuar.</p><a href="#ekosistemi">Shfleto ekosistemin <span><ArrowRight size={17} /></span></a></div></div><div className="ecosystem-orbit" aria-label="Zgjidh një modul"><span className="orbit-core"><Activity /><b>LIVE</b></span>{modules.map(({ icon: Icon, title }, index) => <button type="button" key={title} style={{ "--index": index }} onClick={() => { setActiveModule(index); document.querySelector("#ekosistemi")?.scrollIntoView({ behavior: "smooth" }); }} aria-label={`Shfaq modulin ${title}`}><Icon /><small>{title}</small></button>)}</div></div></section>
